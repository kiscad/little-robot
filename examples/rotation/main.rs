...
fn rotate_to_player_system(
  ...
  for (config, mut enemy_transform) in &mut query {
    // get the enemy ship forward vector in 2D
    ...
    // if the dot product is proximately 1.0 then the enemy is already facing the player
    // and we can early out.
    ...
    // if the dot product is negative then we need to rotate counter clockwise, if it is
    // positive we need to rotate clockwise. Note that `copysign` will still return 1.0 if
    // the dot product is 0.0.
    let right_dot_player = enemy_right.dot(to_player);

    // determine the sign of rotation from the right dot player. we need to negate the sign
    // here as the 2D bevy coordinate system rotates around +Z, which is pointing out of the
    // screen. Due to the right hand rule, positive rotation around +Z is counter clockwise
...
fn snap_to_player_system(
  ...
  for mut enemy_transform in &mut query {
    // get the vector from the enemy ship to the player ship in 2D and normalize it
    let to_player = (player_translation - enemy_transform.translation.xy()).normalize();
    // get the quaternion to rotate from the initial enemy facing direction to
...
fn player_movement_system(
  ...
  // get the distrance the ship will move based on direction, the ship's movement speed and delta time
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
  ...
  let enemy_a_handle = asset_server.load("textures/enemy_A.png");
  let enemy_b_handle = asset_server.load("textures/enemy_B.png");
